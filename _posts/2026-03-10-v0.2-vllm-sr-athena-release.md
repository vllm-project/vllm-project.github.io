...
Since v0.1 Iris, vLLM Semantic Router has been evolving from a routing layer into a broader system brain for mixture-of-modality deployments. What began as signal-driven model selection now extends across safety, semantic caching, memory, retrieval, and tool-aware workflows, turning routing into a programmable control layer between users and models.

Athena makes that shift concrete. Built on a refreshed model foundation, v0.2 expands the system across model selection, stateful memory, hybrid retrieval, programmable DSL language, ClawOS orchestration, zero-config onboarding, AMD-ready deployment, and a much stronger dashboard. If Iris established the bridge between users and models, Athena turns that bridge into a strategic operating surface.
