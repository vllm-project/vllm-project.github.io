...
Athena makes that shift concrete. Built on a refreshed model foundation, v0.2 expands the system across model selection, **ClawOS** orchestration for OpenClaw, stateful memory, hybrid retrieval, richer signals, a programmable DSL language, zero-config onboarding, AMD-ready deployment, and a much stronger dashboard. If Iris established the bridge between users and models, Athena turns that bridge into a strategic operating surface.
...
### 3. ClawOS Explores How Semantic Router Can Power Multiple OpenClaw Systems

One of the biggest new bets in Athena is an **experimental capability** that expands the project in a new direction: **ClawOS**.
...
- **built-in MCP integration** so users can create and manage large numbers of teams and workers (claws) directly through chat
