...
Besides the model structure, different hardware platforms have different attention backend implementations. Even in the same hardware platform, there are some different attention backends to support different user scenarios delivering different performance. In this post, we will introduce the AMD's ROCm attention backend in vLLM. We will elaborate the principles of the attention backend implementation, especially the ROCM_AITER_FA and ROCM_AITER_MLA, along with the corrsponding benchmark performance. This post aims to provide a reference for vLLM users to select the best attention backends on AMD platform.  

In vLLM for AMD ROCm backend, there are four MHA implementations and three Multi-Latent Attention (MLA) implementations. For MHA, they are `TRITON_ATTN`, `ROCM_AITER_UNIFIED_ATTN`, `ROCM_ATTN`, and the `ROCM_AITER_FA`. For MLA, they are `TRITON_MLA`, `ROCM_AITER_TRITON_MLA` and `ROCM_AITER_MLA`. vLLM provides the argument of `--attention-backend` to select the corresponding attention backends which can be summarized as in the following table for the attention selection on AMD's platform.
...
As descrbed above, there are four kinds of MHA attention implementations on AMD ROCm backend. The `TRITON_ATTN`, `ROCM_AITER_UNIFIED_ATTN`, `ROCM_ATTN` are the unified attention, computing attention for prefill, extend and decode tokens in a single kernel.
...
vLLM ROCm backend now supports three unified attention implementations: one is [Triton Unified Attention Backend](https://github.com/vllm-project/vllm/blob/main/vllm/v1/attention/backends/triton_attn.py), which is developed by vllm community, and the other is [AITER Unified Attention Backend](https://github.com/vllm-project/vllm/blob/main/vllm/v1/attention/backends/rocm_aiter_unified_attn.py). Both the implementations follow the same principle but use the different kernels — the former invokes the default [vLLM Triton Attention kernel](https://github.com/vllm-project/vllm/blob/main/vllm/attention/ops/triton_unified_attention.py#L735), while the latter calls the [AITER Triton Attention kernel]().
