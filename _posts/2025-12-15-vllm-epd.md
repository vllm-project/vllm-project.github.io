...
author: "ZHENG Chenguang, Nguyen Kha Nhat Long, Tai Ho Chiu Hero, Le Manh Khuong"
---
...
<p align="center">
<img src="/assets/figures/2025-12-15-epd/workflow.svg" alt="EPD Dataflow Graph" width="80%">
</p>
...
- `start_load_caches(metadata)`: load caches on the PD side before prefill/decoder execution
...
This architecture provides a practical blueprint for next‑generation high‑performance LMM serving systems.
...
Before exploring encoder disaggregation, it's worth noting that **vLLM pioneered the hybrid parallelism strategy for multimodal models on a single node**. The insight is simple but powerful: ViT encoders are small models where tensor parallelism (TP) introduces more overhead than benefit, while LLMs benefit significantly from TP.

vLLM introduced the [ViT Data Parallel + LLM Tensor Parallel](https://github.com/vllm-project/vllm/issues/22743) approach, where the vision encoder runs with data parallelism across GPUs while the language model uses tensor parallelism. This hybrid strategy dramatically reduces TTFT and improves overall throughput. The approach has since been adopted by other serving frameworks, including [SGLang](https://github.com/sgl-project/sglang/pull/13126).

### Prior Art and Industry Adoption

NVIDIA Dynamo has supported EPD-style disaggregation with vLLM internally for some time, though this capability has not yet been released to the open-source community. The vLLM native EPD implementation ([PR #25233](https://github.com/vllm-project/vllm/pull/25233)) was merged in early November 2025, bringing first-class encoder disaggregation support to the open-source ecosystem.
...
Special thanks to Roger Wang, Nicolò Lucchesi, and Cyrus Leung for their valuable feedback and careful reviews during code integration.