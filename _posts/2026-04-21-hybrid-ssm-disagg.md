...
- **3-read conv transfer** --- a decomposition of the Mamba conv state that enables heterogeneous tensor-parallel transfers without reshuffling data on the sender side.
...
This is tracked via the `_mamba_phys_ratio` field, which is computed per-engine (since P and D may have different ratios when their TP sizes differ). The block-ID-to-descriptor-ID mapping in `_get_block_descs_ids` uses the appropriate stride depending on whether it is resolving an FA group or a Mamba group.
...
Each D rank can now read its slice of `x`, `B`, and `C` with three separate, contiguous RDMA reads --- hence "3-read transfer" (we still only issue one 1 nixl READ).
...
3. Mamba descriptors are appended: 6 regions x `N_logical` blocks, with 4 sub-regions each (x, B, C, SSM) for the 3-read transfer.
...
- **Mamba1 models**: The 3-read conv transfer currently supports Mamba2 only. Mamba1's SSM temporal shape `(intermediate_size // tp, state_size)` does not allow reconstructing `intermediate_size`, which is needed for the conv decomposition.
