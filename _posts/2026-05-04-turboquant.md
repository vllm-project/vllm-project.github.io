...
However, most of the reported results were based on small models evaluated on short-context benchmarks that do not stress-test KV-cache quantization. To provide the community with more actionable data, we conduct a comprehensive study spanning four models (both decoder-only and MoEs), from 30B to 200B+ parameters, and five benchmarks including both prefill-heavy long-context retrieval and decode-heavy reasoning workloads.

The results reveal a nuanced tradeoff: **TurboQuant delivers substantial KV-cache memory savings (up to 4.2x over BF16) but consistently reduces throughput by 20-35% due to the computational overhead of dequantization.** In contrast, FP8 KV-cache quantization matches BF16 throughput while providing 2x memory savings. Figures 1-2 summarize this tradeoff as Pareto frontiers of KV-cache capacity versus serving throughput.
...
<br>
<em>Figure 3: Long-context retrieval results for Llama-3.3-70B-Instruct up to 64k context. At 128k, model's maximum supported context length, the BF16 baseline collapses to <10%. </em>
</p>
...
On Qwen3-30B-A3B-Thinking-2507 (Figure 5), we see a clear accuracy hierarchy. FP8 and TQ k8v4 are close to BF16 baseline with >98% average accuracy recovery. TQ 4bit-nc shows a bit higher drop with 96% recovery, whereas TQ k3v4-nc and 3bit-nc show drastic accuracy drops of ~20 points. Even on a relatively easy MATH500 benchmark, the accuracy drop is ~4 points, indicating that aggressive TurboQuant variants are not suitable for long-generation reasoning tasks.
...
<br>
<em>Figure 9: Average throughput relative to BF16 for Qwen3-30B-A3B-Instruct-2507.</em>
</p>

<p align="center">
<img src="/assets/figures/2026-05-04-turboquant/llama_70b_throughput.png" width="100%">
<br>
<em>Figure 10: Average throughput relative to BF16 for Llama-3.3-70B-Instruct.</em>
</p>
...
**Takeaway:** TurboQuant consistently underperforms by reducing throughput and increasing per-token latency compared to both BF16 and FP8. However, in memory-constrained serving scenarios, the KV-cache compression prevents memory saturation and dramatically reduces TTFT under burst load relative to BF16. This is the core of TurboQuant's value proposition: it trades per-token speed for the ability to serve requests that would otherwise be queued. FP8, on the other hand, provides the best of both worlds: it matches or outperforms BF16 throughput while providing negligible latency overhead and significantly better TTFT under burst load.

## Token Efficiency
TODO: measure number of tokens generated by bf16, fp8, and turboquant variants across various decode-heavy benchmarks.
...
**Use FP8 (`--kv-cache-dtype fp8`) as the default.** FP8 provides 2x KV-cache capacity with no throughput cost, minimal accuracy loss (< 1-2 points), and sometimes even improved performance via quantized attention. It is the safest and most predictable choice for the vast majority of workloads, as also detailed in the [FP8 KV-cache blog post](https://vllm.ai/blog/fp8-kvcache). Compared to TurboQuant's k8v4 variant, it is a consistent winner across all metrics except for the small additional KV-cache savings (2.4x vs 2x) which are not worth the trade-off.

**Use TurboQuant 4bit-nc (`--kv-cache-dtype turboquant_4bit_nc`) when you need more than 2x memory savings.** This variant provides up to 3.4x KV-cache capacity with modest accuracy degradation of 1-4 points on most benchmarks. It is particularly valuable for memory-constrained deployments where the TTFT improvement at burst load outweighs negative impacts on all other metrics. Thoroughly validate accuracy on the target workload before deploying.

**Avoid TurboQuant k3v4-nc and 3bit-nc without thorough validation.** These aggressive variants can cause drastic accuracy drops which go up to 20 points on challenging math and coding benchmarks. In addition to accuracy, consistent performance degradation due to complex dequantization steps render them unsuitable for production deployments.

**Stay with BF16 when GPU memory is not a bottleneck.** If your workload uses short contexts, runs at low concurrency, or your hardware has ample memory, BF16 gives the best accuracy and performance trade-off without the risk of quantization artifacts.
