...
- The bounded engines are close at concurrency 1, but overlap exposes their different scheduling and memory policies. vllm-metal's TTFT rises from 10.1 s to 14.7 s while completing all 100 requests; oMLX's SSD mode climbs from 7.8 s to roughly 27 s, while bounded oMLX rejects 37 requests at concurrency 4.
- Earlier first tokens do not guarantee the earliest completion. At concurrency 4, oMLX's SSD arm finishes requests about five seconds sooner on average than vllm-metal. vllm-metal's chunked prefills share each step with active decodes, trading some completion time for earlier admission.
...
At concurrency 16, vllm-metal schedules all requests together and reaches 1.9 s TTFT. llama.cpp's four default slots leave twelve requests queued, pushing TTFT to 28.3 s. The MTP arm turns the same continuous batch into the highest output throughput in the figure, at 71.6 tokens per second.
...
| Concurrency | Wall vs. no MTP | Output tok/s vs. no MTP | TTFT avg vs. no MTP | Acceptance |
|---:|---:|---:|---:|---:|
| 1 | +1% | −3% | +5% | 73.4% |
| 8 | −21% | +23% | +15% | 73.5% |
| 16 | −12% | +13% | +16% | 73.3% |

Single-stream, MTP is a wash on this model because the drafter's cost roughly cancels the accepted tokens. At concurrency 8, batched drafting and verification cut mean request latency from 13.7 s to 10.9 s. Acceptance stays near 73% across the sweep, so batching economics rather than draft quality drives the gain. The tradeoff is TTFT, which rises by about 15% under load. Today the Metal MTP path is limited to Gemma 4 and requires `--no-async-scheduling`; the quickstart above includes both. MTP is opt-in through `--speculative-config`, so prefill-dominated deployments leave it off.
...
- Hybrid-attention models: the Qwen3.5, Qwen3.6, Qwen3.8, and Qwen3-Next families alternate standard attention with gated-delta-net linear attention; v0.28.0 serves `mlx-community/Qwen3.8-27B-8bit` on a single Mac.
...
<!-- TODO before publish: fill in the M1 Pro placeholders from an agent-split run. -->
...
Stats cover completed requests; an empty response counts as failed. The benchmark code and per-engine configurations live in the [SiliconBench repo](https://github.com/WindChimeRan/SiliconBench). The MTP measurements predate v0.4.0 and ran vllm-metal `0.3.0.dev20260821152549` with the serve command from the quickstart.
