...
Padding is free when prompts are the same length and expensive when they are not. To measure only that, we ran two batches of three requests carrying the same total work, about 5,950 prompt tokens and 20 output tokens each, changing nothing but the longest prompt:

| Batch | Prompts | Rows through the model | mlx_lm | oMLX | llama.cpp | vllm-metal |
|---|---|---:|---:|---:|---:|---:|
| A | 2,000 × 3 | 5,973 | 3.32 s | 6.32 s | 4.67 s | 3.49 s |
| B | 4,000 + 1,000 + 1,000 | 11,970 | 5.72 s | 5.69 s | 4.68 s | 3.65 s |
| | | **change** | **+72%** | −10% | 0% | +5% |

Only mlx_lm moves, and by what the shape predicts: its prefill doubles while its decode stays fixed. On batch A it is the fastest engine in the table, because with equal lengths padding costs nothing and batching is pure gain. The same mechanism makes it the slowest on batch B. oMLX is flat because it never batches a prefill, and pays for that by being slowest when lengths do match.

A padded batch ties throughput to the shape of the traffic. Packing breaks that link. On the agent split, prompt lengths span 305 to 10,076 tokens, and a server does not get to choose them. Padding sizes memory the same way, by the longest sequence in the batch, while a paged pool is sized once at startup.
...
The padding A/B in "Packed queries and paged KV" is a separate, smaller measurement: three requests sent at once to a freshly started server, 20 output tokens each, one run per cell. Batch A and batch B carry the same total prompt tokens, so the only variable is the longest prompt. TTFT spread across the three requests confirms whether an engine actually placed them in one batch: mlx_lm's spread is 0.03 s or less in both batches, oMLX's is over 1.5 s.
