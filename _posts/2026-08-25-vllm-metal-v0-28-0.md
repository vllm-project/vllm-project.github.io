...
- **oMLX (SSD offload):** `--paged-ssd-cache-dir <fresh-empty-dir>` and `--hot-cache-max-size 0`, with a restart and fresh directory before each concurrency level. This is oMLX's own default apart from the directory, which has to be fresh so no level inherits the previous one's cache.
- **oMLX (RAM cache):** the same, plus `OMLX_HOT_CACHE_ONLY=true` and `--hot-cache-max-size 8GB`, which keeps the prefix cache in memory and writes nothing to disk. The cache directory is still required — oMLX builds no cache without one — but stays empty.
