...
![SiliconBench agent split on Qwen3.6-35B-A3B: TTFT, end-to-end request latency, and output token throughput versus concurrency for llama.cpp, vllm-metal, and oMLX with and without SSD offload](/assets/figures/2026-08-25-vllm-metal-v0-4-0/moe-35b-a3b.svg)

vllm-metal holds TTFT nearly flat, 1.6 s to 2.0 s from concurrency 1 to 4, while both oMLX modes climb to about 6.5 s and llama.cpp sits between them. Output throughput reaches 35.3 tokens per second against 30.1 for oMLX and 25.3 for llama.cpp. oMLX is marginally ahead single-stream on latency and throughput, and behind everywhere requests overlap.
