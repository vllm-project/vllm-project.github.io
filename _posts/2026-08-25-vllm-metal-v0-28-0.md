...
| Batch | Prompt tokens | mlx_lm | oMLX | lily | llama.cpp | vllm-metal |
|---|---|---:|---:|---:|---:|---:|
| A | 750 × 8 | 4.52 | 7.32 | 4.26 | 5.29 | 3.87 |
| B | 3,000 + 430 × 7 | 10.99 | 7.22 | 4.30 | 5.33 | 3.64 |
| **Change** | | **+143%** | −1% | +1% | +1% | −6% |

[lily](https://github.com/perplexityai/pplx-garden/tree/main/lily) serves one request at a time, so its time is the total for eight sequential requests.
...
The padding comparison reports the median of two runs per cell, with a fresh server for each run. Each run sends eight requests with 20 output tokens each; lily's requests are sent sequentially. The NAX A/B uses Qwen3-0.6B with two Sonnet configurations: 2,048 input / 32 output tokens and 1,024 input / 128 output tokens. Both run 100 prompts at request rate 10 and concurrency 32.

The benchmark code, configurations, and result files live in the [SiliconBench repo](https://github.com/WindChimeRan/SiliconBench). The serving comparisons use vllm-metal `0.28.0.dev20260901062632` with vLLM 0.28.0, llama.cpp `0eadefeb`, and oMLX `dc312e6e`.
