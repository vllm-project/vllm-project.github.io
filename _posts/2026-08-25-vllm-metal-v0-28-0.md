...
Serving engines batch prefill attention differently:

| Engine | Prefill attention | Query axes | KV |
|---|---|---|---|
| [Lily](https://github.com/perplexityai/pplx-garden/tree/main/lily) | per-prompt calls | `[1, T]` | contiguous |
| [oMLX](https://github.com/jundot/omlx) | per-prompt calls | `[1, T]` | contiguous |
| [Uzu](https://github.com/trymirai/uzu) | per-prompt calls | `[1, T]` | contiguous |
| [Splash](https://github.com/incoai/splash) | per-prompt calls | `[1, T]` | paged |
| [mlx_lm](https://github.com/ml-explore/mlx-lm) | padded, up to 8 prompts | `[B, T_max]` | contiguous |
| [llama.cpp](https://github.com/ggml-org/llama.cpp) | mask over slots | `[total_q]` | fixed cells |
| [vllm-metal](https://github.com/vllm-project/vllm-metal) | packed, `cu_seqlens` | `[total_q]` | paged |

*Here `B` is prompt count, `T` query length, `T_max` the longest query, and `total_q` the sum; head axes are omitted. Per-prompt calls describe attention only, so projections may still be batched. Flat query arrays keep prompts separate through masks or `cu_seqlens`. KV describes logical storage: contiguous buffers, individual token cells, or token blocks.*
