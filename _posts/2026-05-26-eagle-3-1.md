...
At the same time, backward compatibility with existing EAGLE 3 checkpoints is fully preserved. As a result, EAGLE 3.1 draft models can be deployed directly through the same speculative decoding interface already used in production vLLM serving systems.

Detailed benchmark results will be shared in a follow-up update.

The support has already been merged into the current vLLM main branch and will be available through nightly releases as well as the upcoming **v0.22.0** release.
