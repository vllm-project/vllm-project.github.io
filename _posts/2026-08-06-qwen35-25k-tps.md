...
image: /assets/figures/2026-08-06-qwen35-25k-tps/pareto-frontier-qwen35-nvfp4.png
social_image: /assets/figures/2026-08-06-qwen35-25k-tps/pareto-frontier-qwen35-nvfp4.png
tags:
  - performance
  - qwen3.5
  - disaggregation
...
[[Core][KVConnector] Support HMA+NixlConnector #35758](https://github.com/vllm-project/vllm/pull/35758) and [PD disagg with NIXL Connector: GDN support (Qwen3.5) #41869](https://github.com/vllm-project/vllm/pull/41869)

Previously, KV connectors could not use vLLM's Hybrid Memory Allocator (HMA) and instead treated all layers as conventional full-attention layers. This would be very inefficient for hybrid models.

By mapping HMA's logical blocks onto the correct physical memory regions, the change allows NIXL to transfer only the cache regions belonging to each layer type. HMA reduced the number of transferred descriptors from 4,284 to 1,650 and improved throughput by up to approximately 7% in a small-scale intra-node H100 setup.
...
* `VLLM_SSM_CONV_STATE_LAYOUT=DS` together with `--no-disable-hybrid-kv-cache-manager` — a mandatory pair for SSM models in disaggregated serving; the conv-state transfer does not work without it.
