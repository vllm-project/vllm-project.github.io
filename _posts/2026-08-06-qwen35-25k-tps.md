...
For performance measurements we used a fixed input sequence length / output sequence length benchmark, on a random dataset with `random_range_ratio=0.8`. The recipes themselves, and the settings that matter most, are covered in [Recipes & best practices](#recipes--best-practices) below.
