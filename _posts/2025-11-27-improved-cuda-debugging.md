...
Then, we can run the code and trigger the core dump:
...
While the code is running forever, and we suspect it is hanging in the `conditional_hang_kernel`, we can trigger the core dump by writing to the pipe:

```bash
dd if=/dev/zero bs=1M count=1 > /tmp/cuda_coredump_pipe_hostname.3000837.1764236276
```

Here we write 1MB of zeros to the pipe, which will trigger the core dump. Simple `echo aaa > /tmp/cuda_coredump_pipe_hostname.3000837.1764236276` might not work due to the buffering of the pipe.
